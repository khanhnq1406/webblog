...
export async function generateMetadata({
  ...
  try {
    const { frontmatter } = getContentBySlug("fe-interview", params.slug);
...
export default function FeInterviewSlugPage({
  params,
}: {
  params: { slug: string };
}) {
  ...
  try {
    const { content, frontmatter } = getContentBySlug("fe-interview", params.slug);
